...
const float ver = 1.0;  // 宣告全域浮點常數：程式版本號
int ID, mode;  // 宣告全域整數變數函數：使用者身分、模式選擇代碼
int m20, m25, m30, m35, m40, m45, m50, m55, m60, m65;  // 宣告全域整數變數函數：北捷各個票價的搭乘次數
...
void main()  // 定義主程式區塊
{
	m20 = m25 = m30 = m35 = m40 = m45 = m50 = m55 = m60 = m65 = 0;  // 定義 北捷 各個票價的搭乘次數 初始值為 0
	...
	printf("KPPT 1200 TPASS Calculator Ver%3.1f ( C Language ) Made by CHE_72 ZStudio\n", ver);  // 輸出版權訊息
	...
	case 1:  // 模式1：臺北捷運
	{
		...
		zero_times();  // 輸出 0繼續 訊息
		  /* 調用 臺北捷運 資料輸入函式 */
		m20 = tpmrt(20), m25 = tpmrt(25), m30 = tpmrt(30), m35 = tpmrt(35), m40 = tpmrt(40);
		m45 = tpmrt(45), m50 = tpmrt(50), m55 = tpmrt(55), m60 = tpmrt(60), m65 = tpmrt(65);
		tpmrt_times = tpmrt_times + m20 + m25 + m30 + m35 + m40 + m45 + m50 + m55 + m60 + m65;
		  // 計算用戶搭乘 臺北捷運 的總次數（若使用者曾輸入過，原數據不會被覆蓋）
		tpmrt_fare = tpmrt_fare + m20 * 20 + m25 * 25 + m30 * 30 + m35 * 35 + m40 * 40 + m45 * 45 + m50 * 50 + m55 * 55 + m60 * 60 + m65 * 65;
		  // 計算用戶搭乘 臺北捷運 的總票價（若使用者曾輸入過，原數據不會被覆蓋）
